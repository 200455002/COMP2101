...
echo "An error occured. please try again after finding a problem."
}
...
cat <<EOF
NAME OF THE SCRIPT:
  sysconfig.sh - display all system information
HOW TO USE?
  sysconfig.sh [OPTION]
OPTION:
  -h or --help
  ...
  --host
        Only presents host name
  --domain
        Only presents NIS/YP domain name
  --ipconfig
        Only presents current machine's IP info and all network interfaces
  --os
        Only presents current machine's OS name and version
  --cpu
        Only presents current machine's hardware information
  --memory
        Only presents current machine's memory information
  --disk
        Only presents current machine's disk usage information
  --printer
        Only presents current machine's printer information
NO OTHER OPTIONS WILL NOT BE RECOGNIZED
EOF
}
...
function cleanup {
	rm -rf /tmp/*info*
	echo "cleanup exucution completed."
}
